...
class Game:
    ...
    # ===============================
    # 🤖 YAPAY ZEKA (AI) DEĞİŞKENLERİ
    # ===============================
    self.ai_comment = "Yapay zeka yorumu hazırlanıyor..."
   ...
   def draw_game_over(self):
    ...
    restart_text = self.small_font.render(
        "Yeniden başlamak için R tuşuna basın", True, WHITE
    )
    quit_text = self.small_font.render(
        "Çıkmak için ESC tuşuna basın", True, WHITE
    )

    # Temel yazılar
    self.screen.blit(game_over_text, (WIDTH // 2 - 100, HEIGHT // 2 - 120))
    self.screen.blit(score_text, (WIDTH // 2 - 120, HEIGHT // 2 - 80))
    self.screen.blit(high_score_text, (WIDTH // 2 - 120, HEIGHT // 2 - 50))

    # Yeni rekor efekti
    if self.new_record:
        new_record_text = self.title_font.render("YENİ REKOR!", True, YELLOW)
        new_record_x = WIDTH // 2 - new_record_text.get_width() // 2
        self.screen.blit(new_record_text, (new_record_x, HEIGHT // 2 - 170))

    # ===============================
    # 🤖 YAPAY ZEKA YORUMU (AI)
    # ===============================
    ai_surface = self.small_font.render(
        self.ai_comment, True, (180, 200, 255)
    )
    ai_x = WIDTH // 2 - ai_surface.get_width() // 2
    ai_y = HEIGHT // 2 + 10
    self.screen.blit(ai_surface, (ai_x, ai_y))

    # Alt yazılar
    self.screen.blit(restart_text, (WIDTH // 2 - 180, HEIGHT // 2 + 50))
    self.screen.blit(quit_text, (WIDTH // 2 - 140, HEIGHT // 2 + 80))
    ...
    # ===============================
    # 🤖 AI ÇAĞRISI (DOĞRU YER)
    # ===============================
    if self.game_over and not self.ai_requested:
        self.ai_requested = True
        import threading
        threading.Thread(
            target=self.request_ai_comment,
            daemon=True
        ).start()
